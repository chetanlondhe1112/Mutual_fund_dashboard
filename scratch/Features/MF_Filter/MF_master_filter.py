...
# Testing file generation :Combining Rolling returns file with main file

test_df=selected_file_df.copy()
...
if st.checkbox("Apply"):
        ...
        if condition_2=="Above Average":
            ...
            for value in test_column_df.to_list():
                ...
                if value < average:
                    ...
                    arr.append(0)
        elif condition_2=='Gold':
            for value in test_column_df.to_list():
                if value == 'Gold':
                    arr.append(weightage_1)
                else:
                    arr.append(weightage_2)
    ...
    show_df.set_index([pd.Index(x), 'Legal_Name'],inplace = True)
    st._legacy_dataframe(show_df)



    st.subheader("Top 10 Funds")
    top_fund_df=total_weightage[['Legal_Name','Result']].sort_values(by='Result',ascending=False).head(10)
    Top_Weighatge_file=top_fund_df.to_csv()
    st.download_button("📥Export",key=1, data=Top_Weighatge_file, file_name='Top 10 Funds:'+selected_file_name+'.csv')
