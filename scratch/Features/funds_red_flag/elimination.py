...
with layout_col[1]:
    st.subheader("Funds")
    st._legacy_dataframe(selected_file_df['Legal_Name'])
    
    st.subheader("Parameters list")
    parameters_list=selected_file_df.columns.to_list()[4:]
...
for x in match_sheet_parameter:
    st.write(selected_file_df[x])        

with layout_col[0]:
    st.subheader("ELiminated bad funds:")
    df2=selected_file_df.loc[(selected_file_df[match_sheet_parameter]!=0).all(1)]
    st.write("Results:{}".format(len(df2)))
    st._legacy_dataframe(df2)
